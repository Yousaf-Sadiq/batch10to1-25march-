...
if (isset($_POST["delete"]) && !empty($_POST["delete"])) {
  ...
  if ($exe_allData->num_rows > 0) {
    ...
    $address_delete_check = "SELECT * FROM `user_address` WHERE `user_id`='{$user_id}'";

    $address_delete_exe = $conn->query($address_delete_check);



    if ($address_delete_exe->num_rows > 0) {

      $fetchAdress = $address_delete_exe->fetch_assoc();

      if (isset($fetchAdress["image"]) && !empty($fetchAdress["image"])) {

        $oldImage = json_decode($fetchAdress["image"], true);
      ...
      $addressDelete = "DELETE FROM `user_address` WHERE `user_id`='{$user_id}' ";


      $delete_exe = $conn->query($addressDelete);
    ...
    $delete_exe = $conn->query($userDelete);
    ...
    if ($delete_exe) {
      ...
      if ($conn->affected_rows > 0) {
        ...
        ERROR_MSG("DATA HAS NOT BEEN DELETED  {$userDelete}");
      }
    ...
    ERROR_MSG("RECORD IS NOT EXISTED");
...
if (isset($_POST["login"]) && !empty($_POST["login"])) {
  ...
  if ($status["error"] > 0) {
    ...
    $check_email = "SELECT * FROM `users` WHERE `email`='{$email}' AND `ptoken`='{$encrpyt}'  ";
    ...
    if ($exe_email->num_rows > 0) {
      ...
      $fetch_email = $exe_email->fetch_assoc();

      $_SESSION["Email"] = $fetch_email["email"];
      $_SESSION["user_id"] = $fetch_email["user_id"];

      Success_msg("LOGIN SUCCESSFULL");

    } else {
      ERROR_MSG("YOU ARE NOT REGISTER IN OUT PORTAL OR MAYBE EMAIL OR PASSWORD MAY WRONG");
    }
...
if (isset($_POST["signUP"]) && !empty($_POST["signUP"])) {
  ...
  if ($status["error"] > 0) {
    ...
    $insert = "INSERT INTO `users`( `email`,`user_name`,`password`, `ptoken`) 
  VALUES ('{$email}','{$user_name}','{$hash}','{$encrpyt}')";
